...
type RestOf<T extends IBook | NewBook> = Omit<T, 'releaseDate'> & {
...
export class BookService {
  ...
  protected convertDateFromClient<T extends IBook | NewBook | PartialUpdateBook>(book: T): RestOf<T> {
    return {
      ...book,
      releaseDate: book.releaseDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restBook: RestBook): IBook {
    return {
      ...restBook,
      releaseDate: restBook.releaseDate ? dayjs(restBook.releaseDate) : undefined,
    };
  }
